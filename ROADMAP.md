...
- [x] OpenAI 兼容端点：OpenAI / DeepSeek / NewAPI 网关（`chatapi/ConnectionParams.py`、`OpenAIConnectionHandler`）
...
- [ ] ❗ CoT 统计/分析工具：扫描 `chatapi/history/` 或批处理产出的 jsonl，统计每次回复的 reasoning 长度、reasoning/正文比例等，用于量化不同供应商的推理链风格差异
...
- [x] vLLM Python 全托管：新建 `chatapi/VLLMServer.py`，`subprocess` 启动 `vllm serve`，轮询 `/health` 等待就绪（超时抛出并附带子进程输出尾部），支持上下文管理器 + `atexit` 自动清理；`connection_params()` 直接返回配套的 `VLLMConnectionParams`
...
- [x] `Conversation` 会话编排（`chatapi/Conversation.py`）：串联 Prompt、历史记录与连接层
- [x] `MessageHistory` 会话 JSON 落盘格式（`chatapi/MessageHistory.py`）：`version`/`session`/`metadata`/`messages` 结构，支持 `load()`/`save_to()`/`save_to_jsonl()`
- [ ] ❗ 会话加载与续聊（resume）入口：`MessageHistory.load()`/`Conversation.from_history()` 已经实现，但缺一个现成的脚本/CLI 入口——"读取已保存的 session 文件 → 继续对话"，目前只能手写代码调用

## Prompt 管理

- [x] `Prompt` / `PromptManager` 模板机制（`chatapi/Prompt.py`、`chatapi/prompts/prompts.json`）：多语言、`{{var}}` 占位符替换
- [x] `PROMPTS_DIR` 环境变量：`PromptManager` 默认加载包内 `chatapi/prompts/prompts.json`，若设置了 `PROMPTS_DIR` 则改为加载 `$PROMPTS_DIR/prompts.json`，让打包后下游项目可以整套替换成自己的 prompt 库而不用改代码
...
- [x] 依赖管理与打包（`pyproject.toml`）：`chatapi` 作为可安装包（`chatapi`），锁定 `openai`/`google-genai`/`pandas`/`tqdm`/`pyparsing` 的大版本区间；`chatapi/prompts/*.json`、`*.txt` 通过 `package-data` 随包分发；版本号单一来源于 `pyproject.toml`，`chatapi/__init__.py` 通过 `importlib.metadata` 读取暴露为 `__version__`。支持 `pip install git+https://git.epsiotapi.com/EpsIotaPi/ChatAPI.git`（含 `@<tag>` 锁版本）从私有 Git 服务器安装到其它项目/远程服务器。副带修复：`PromptManager` 默认路径及 `prompts.json` 里的 `path_system`/`path_user` 原本是 CWD 相对路径（且有 `chatapi/prompt/` 目录名 typo），装成包后从别的项目调用会 `FileNotFoundError`；改为基于包内路径解析，已在临时 venv 中验证跨目录导入正常
